...
class Dealer():
    ...
    # Evaluate the ranking of a hand of POKER cards.
    ...
    def print_hands(self):
        for i in range(len(self.players_list)):
            print (f"Player {i+1}: {[card.id for card in self.players_list[i].cards]}")
        print (f"Flop: {[card.id for card in self.flop.cards]}")
    ...
    def update(self):
        ...
        if self.dealt_cards < (self.number_players * 2):
            self.deal_hole_cards()

        if self.animating_card:
            self.animate_hole_card(self.animating_card)

        if self.dealt_cards == (self.number_players * 2) and (not self.animating_card or self.animating_card.animation_complete):
            self.can_deal_flop = True
            self.deal_flop()
        
        if self.dealt_cards < (self.number_players * 2) + 3 and self.can_deal_flop:
            self.deal_flop()
        
        if not self.printed_flop and self.dealt_cards == (self.number_players * 2) + 3:
            self.print_hands()
            self.printed_flop = True
        
        if self.dealt_cards == (self.number_players * 2) + 3 and self.determined_winner is None:
